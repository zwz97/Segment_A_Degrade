...
try:
    ...
    print("Error: Could not import from the 'segment_anything' library.")
    print("Please ensure you have cloned the repo and installed it following the official README:")
    print("  git clone https://github.com/facebookresearch/segment_anything.git")
    print("  cd segment_anything")
    print("  pip install -e .")
    print("  cd ..")
...
# --- Model Loading --- 
def load_sam1_predictor_and_generator(model_type: str, checkpoint_path: str, generator_config: dict) -> tuple[SamPredictor | None, SamAutomaticMaskGenerator | None]:
    """Loads the SAM1 model predictor and mask generator.

    Uses the SAM1 library's from_pretrained method, which handles downloading 
    from Hugging Face Hub (if model_type is an ID) or loading from a local 
    checkpoint (.pt) specified by the ID.

    Uses a simple global cache to avoid reloading if called again with the 
    same model_type and generator_config.

    Args:
        model_type: The identifier for the SAM1 model. Can be a Hugging Face 
    ...
    # Check cache
    if (_cached_model == model_type and 
        _cached_generator_config == generator_config and 
        _cached_predictor and 
        _cached_generator):
        print(f"Using cached SAM1 predictor and generator for {model_type}")
        return _cached_predictor, _cached_generator

    print(f"Loading SAM1 model and generator for {model_type}...")
    try:
        # Load the predictor using the sam1 library's method
        # This handles loading from HF Hub or local checkpoints internally
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        sam1_model = sam_model_registry[model_type](checkpoint=checkpoint_path).to(device)
        ...
        generator = SamAutomaticMaskGenerator(sam1_model, **generator_config)
        
        # Update cache
        _cached_model = model_type
        _cached_generator_config = generator_config
        _cached_predictor = predictor
        _cached_generator = generator
        
        print("SAM1 model and generator loaded successfully.")
        return predictor, generator
    except Exception as e:
        print(f"Error loading SAM1 model or generator {model_type}: {e}")
...
def load_sam1_evaluation_data(data_path: str, image_base_dir: str) -> list[dict] | None:
...
if __name__ == "__main__":
    ...
    # --- 2. load_sam1_evaluation_data minimal flow ---
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        items = load_sam1_evaluation_data(data_json_path, tmpdir)
        assert items and len(items) == 1, "load_sam1_evaluation_data failed"
        assert os.path.isabs(items[0]["image_filepath"]), "image_filepath not absolute"
        print("  ✓ load_sam1_evaluation_data minimal flow passed")
